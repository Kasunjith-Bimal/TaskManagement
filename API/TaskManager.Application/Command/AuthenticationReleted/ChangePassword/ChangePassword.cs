...
namespace TaskManager.Application.Command.AuthenticationReleted.ChangePassword
{
    public class ChangePassword : IConsumer<ChangePasswordCommand>
    {
        ...
        public ChangePassword(ILogger<ChangePassword> logger, IConfiguration configuration, IUserService userService, IJwtTokenManager JwtTokenManager)
        {
            this.logger = logger;
                    ...
                    if ((!String.IsNullOrEmpty(context.Message.NewPassword)))
                    {
                        ...
                        if (!String.IsNullOrEmpty(context.Message.Email))
                        {
                            ...
                            if (checkExistingEmployee != null)
                            {
                                ...
                                {
                                    var chekOldPasswordCurrect = await this.userService.CheckPasswordAsync(checkExistingEmployee, context.Message.OldPassword);

                                    if (chekOldPasswordCurrect)
                                    {
                                        var updateNewPassword = await this.userService.ChangePasswordAsync(checkExistingEmployee, context.Message.OldPassword, context.Message.NewPassword);

                                        if (updateNewPassword)
                                        {
                                            var getUpdatedEmployee = await this.userService.GetUserByIdAsync(checkExistingEmployee.Id);

                                            if (getUpdatedEmployee != null)
                                            {
                                                getUpdatedEmployee.IsFirstLogin = false;
                                                var updateEmployee = await this.userService.UpdateUser(getUpdatedEmployee);
